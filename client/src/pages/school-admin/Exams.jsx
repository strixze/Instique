...
export default function Exams() {
  ...
  const columns = [
    { key: 'name', label: 'Exam', sortable: true, render: (r) => <span className="font-medium text-gray-200">{r.name}</span> },
    ...
    { key: 'status', label: 'Status', render: (r) => <Badge color={statusColors[r.status] || 'gray'}>{r.status}</Badge> },
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-1">
          <button onClick={() => openMarks(r)} className="p-2 text-gray-400 hover:text-indigo-400 rounded-lg hover:bg-indigo-500/10 transition-colors" title="View marks">
            <Trophy size={16} />
          </button>
          {r.status !== 'published' && (
            <button onClick={() => handlePublish(r)} className="p-2 text-gray-400 hover:text-green-400 rounded-lg hover:bg-green-500/10 transition-colors" title="Publish results">
              <Send size={16} />
            </button>
          )}
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={open} onClose={resetAndClose} title="Create Exam" size="lg">
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-gray-300">Subjects *</h3>
          ...
          <div className="space-y-3">
            {form.subjectRows.map((row, index) => (
              <div key={index} className="flex items-end gap-2">
                ...
                <button onClick={() => removeRow(index)} className="p-2 text-gray-400 hover:text-red-400 rounded-lg hover:bg-red-500/10 transition-colors">
                  <Trash2 size={16} />
                </button>
              </div>
            ))}
      ...
      <Modal isOpen={!!marksFor} onClose={() => setMarksFor(null)} title={`Marks — ${marksFor?.name || ''}`} size="lg">
        {marksLoading ? (
          <div className="h-32 flex items-center justify-center text-gray-500">Loading marks...</div>
        ) : marks.length === 0 ? (
          <p className="text-center text-gray-500 py-8">No marks have been entered for this exam yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-700">
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Student</th>
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Subject</th>
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Marks</th>
                  <th className="px-3 py-2 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Grade</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-700/50">
                {marks.map((m) => (
                  <tr key={m._id}>
                    <td className="px-3 py-2 text-gray-200">{m.student?.firstName} {m.student?.lastName}</td>
                    <td className="px-3 py-2 text-gray-300">{m.subject?.name}</td>
                    <td className="px-3 py-2 text-gray-300">{m.marksObtained} / {m.maxMarks}</td>
