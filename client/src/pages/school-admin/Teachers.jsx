...
function getInitials(firstName = '', lastName = '') {
  return ((firstName[0] || '') + (lastName[0] || '')).toUpperCase() || 'TC';
}
...
export default function Teachers() {
  ...
  const renderGender = (gender) => {
    if (gender === 'female') return <span className="inline-flex items-center gap-1 text-xs font-medium text-pink-600"><span className="text-sm">&#9792;</span> Female</span>;
    if (gender === 'male') return <span className="inline-flex items-center gap-1 text-xs font-medium text-blue-600"><span className="text-sm">&#9794;</span> Male</span>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:flex-items-center sm:justify-between gap-3 pb-1">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-deep tracking-tight">Teachers</h1>
          <p className="text-secondary text-xs sm:text-sm mt-0.5">Manage teacher profiles, subjects, and assignments</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => setBulkOpen(true)} className="gap-1.5"><Upload size={14} /> Bulk Import</Button>
          <Button onClick={openCreate} className="gap-1.5"><Plus size={16} /> Add Teacher <ChevronDown size={14} className="opacity-70 ml-0.5" /></Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white border border-border rounded-xl p-3.5 shadow-2xs hover:shadow-card transition-shadow">
          <div className="w-8 h-8 rounded-full bg-forest-soft text-forest flex items-center justify-center mb-2"><GraduationCap size={16} /></div>
          <p className="text-[11px] font-semibold text-secondary">Total Teachers</p>
          <p className="text-xl sm:text-2xl font-bold text-deep leading-tight mt-0.5">{totalCount}</p>
          <p className="text-[10px] text-muted mt-1">All staff members</p>
        </div>
        <div className="bg-white border border-border rounded-xl p-3.5 shadow-2xs hover:shadow-card transition-shadow">
          <div className="w-8 h-8 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center mb-2"><Users size={16} /></div>
          ...
          <p className="text-[10px] text-muted mt-1">Currently teaching</p>
        </div>
        <div className="bg-white border border-border rounded-xl p-3.5 shadow-2xs hover:shadow-card transition-shadow">
          <div className="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mb-2"><Building2 size={16} /></div>
          <p className="text-[11px] font-semibold text-secondary">Departments</p>
          <p className="text-xl sm:text-2xl font-bold text-deep leading-tight mt-0.5">{depts}</p>
          <p className="text-[10px] text-muted mt-1">Across campus</p>
        </div>
        <div className="bg-white border border-border rounded-xl p-3.5 shadow-2xs hover:shadow-card transition-shadow">
          <div className="w-8 h-8 rounded-full bg-amber-50 text-amber-600 flex items-center justify-center mb-2"><BookOpen size={16} /></div>
          <p className="text-[11px] font-semibold text-secondary">Subjects Covered</p>
          <p className="text-xl sm:text-2xl font-bold text-deep leading-tight mt-0.5">{subjectCount}</p>
      ...
      <div className="bg-white border border-border rounded-xl p-3.5 shadow-2xs space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <Button variant="outline" size="sm" onClick={handleResetFilters} className="text-xs text-secondary"><RotateCcw size={13} className="mr-1 text-muted" /> Reset</Button>
            <Button size="sm" variant="primary" className="text-xs gap-1.5">
              <Filter size={13} /> Filters
              {countActiveFilters() > 0 && <span className="ml-0.5 px-1.5 bg-white/20 text-white rounded-full text-[10px] font-bold">{countActiveFilters()}</span>}
            </Button>
          </div>
        </div>
            ...
            {STATUS_PILLS.map((pill) => {
              ...
              return (
                <button key={pill.value} onClick={() => { setStatusFilter(pill.value); setPage(1); }}
                  className={`px-3 py-1 rounded-lg text-xs transition-all whitespace-nowrap ${isActive ? 'bg-forest text-white font-semibold shadow-2xs' : 'bg-white border border-border text-secondary hover:bg-surface hover:text-deep font-medium'}`}>
                  {pill.label}
                </button>
              );
            })}
                    ...
                    <tr key={row._id} className="hover:bg-surface/50 transition-colors">
                      ...
                      <td className="px-3.5 py-2.5 text-right relative">
                        <div className="inline-flex items-center gap-1">
                          <button onClick={() => { setActiveMenuId(activeMenuId === row._id ? null : row._id); }} className="p-1.5 text-muted hover:text-deep hover:bg-surface border border-border rounded-lg transition-colors" title="More actions">
                        ...
                        {activeMenuId === row._id && (
                          <div ref={menuRef} className="absolute right-4 top-10 w-40 bg-white border border-border rounded-xl shadow-dropdown z-40 py-1 text-left animate-scale-in">
                            <button onClick={() => { setActiveMenuId(null); openEdit(row); }} className="w-full px-3 py-1.5 text-xs text-deep hover:bg-surface flex items-center gap-2">
