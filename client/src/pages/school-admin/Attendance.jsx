    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 p-1 bg-gray-800 border border-gray-700 rounded-xl w-fit">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'mark' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-gray-200'}`}
        >
          <Users size={14} className="inline mr-1.5 -mt-0.5" />Mark Attendance
        </button>
        <button
          onClick={() => { setActiveTab('history'); setLoading(true); }}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'history' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-gray-200'}`}
        >
          History
        </button>
      </div>
      ...
      {activeTab === 'mark' ? (
        <div className="space-y-5 animate-scale-in">
          ...
          <div className="flex flex-wrap items-end gap-3 p-5 bg-gray-800 border border-gray-700 rounded-2xl">
            <div className="w-44">
          ...
          {loadingStudents ? (
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-2xl overflow-hidden">
              {/* Summary Bar */}
              <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3 border-b border-gray-700/60 bg-gray-800/80">
                <div className="flex items-center gap-4 text-sm">
                  <span className="text-gray-400">Total: <strong className="text-gray-100">{studentList.length}</strong></span>
                  ...
                  <div className="flex items-center gap-1 border border-gray-700 rounded-lg p-0.5">
                    {STATUS_OPTIONS.map((opt) => (
                      <button
                        ...
                        className={`px-2.5 py-1 rounded text-[11px] font-bold transition-colors bg-gray-700 text-gray-300 ${opt.hoverColor}`}
                    ...
                    {studentList.map((student, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-3 font-medium text-gray-200">{student.firstName} {student.lastName}</td>
                          <td className="px-3 py-3 text-gray-400 text-sm">{student.admissionNo || student.rollNo || '—'}</td>
              ...
              {/* Submit bar */}
              <div className="px-5 py-4 border-t border-gray-700/60 bg-gray-800/80 flex items-center justify-between">
            ...
            <div className="flex flex-col items-center justify-center py-16 text-gray-500 bg-gray-800 border border-gray-700 rounded-2xl">
              <Users size={40} className="text-gray-600 mb-3" />
              <p className="font-medium text-gray-300">No students found</p>
              <p className="text-sm mt-1">No active students are assigned to this class and section.</p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center py-16 text-gray-500 bg-gray-800 border border-gray-700 rounded-2xl">
              <CheckCheck size={40} className="text-gray-600 mb-3" />
              <p className="font-medium text-gray-300">Select class & section to begin</p>
