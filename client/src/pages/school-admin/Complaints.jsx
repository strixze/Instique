...
export default function Complaints() {
  ...
  const columns = [
    { key: 'subject', label: 'Subject', sortable: true, render: (r) => <span className="font-medium text-gray-200">{r.subject}</span> },
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={!!processing} onClose={() => setProcessing(null)} title={`Process Complaint — ${processing?.subject || ''}`}>
        <div className="space-y-4">
          <p className="text-sm text-gray-400">{processing?.description}</p>
          <Select
            label="Status"
            options={[
          ...
          <div>
            ...
            <textarea
              className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent min-h-24"
