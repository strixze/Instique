    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
      ...
      >
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-border/80 dark:border-dark-border shrink-0">
          <div>
            <h2 className="text-base font-bold text-deep dark:text-dark-text leading-tight">{title}</h2>
            {description && (
              <p className="text-xs text-muted dark:text-dark-text-muted mt-0.5">{description}</p>
            )}
          </div>
          <button
            onClick={onClose}
        ...
        {/* Content Body */}
        <div className="p-6 overflow-y-auto flex-1 text-deep dark:text-dark-text">{children}</div>

        {/* Optional Footer */}
        {footer && (
          <div className="flex items-center justify-end gap-3 px-6 py-3.5 border-t border-border/80 dark:border-dark-border bg-slate-50/50 dark:bg-dark-card/50 rounded-b-2xl shrink-0">
            {footer}
          </div>
        )}
      </div>
