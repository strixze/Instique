...
function MessageContextMenu({ open, isSent, onAction, onClose, anchorRef }) {
 ...
 a.danger ? 'text-red-400 hover:bg-danger-light' : 'text-[#c4c6e7] hover:bg-white/[0.06]',
...
function DeleteModal({ show, onClose, onDeleteForMe, onDeleteForEveryone }) {
 if (!show) return null
 return (
 <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center bg-deep/40 backdrop-blur-sm px-4 pb-6 sm:pb-0"
...
function MessageInfoModalGroup({ show, onClose, msg, seenBy }) {
 ...
 return (
 <div
 className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center bg-deep/40 backdrop-blur-sm px-4 pb-6 sm:pb-0"
 style={{ animation: 'fadeIn 0.15s ease' }}
 >
 <div
...
function Message({ msg, key, isGroupChat, onReply = () => { } }) {
 ...
 <div className="text-[12px] text-muted bg-surface/40 px-3 py-1 rounded-full">
 {msg.message}
 </div>
 </div>
 )
 ...
 {/* Text */}
 {hasText && (isLink ?
 <div className={`text-[13.5px] leading-[1.55] px-3.5 pt-2.5 font-[Sora,sans-serif] ${!hasImage ? 'pb-[26px]' : 'pb-6'} text-info-text hover:underline`}>
