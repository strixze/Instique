...
export const createManualTimetable = async (schoolId, data) => {
  const {
    schoolClass,
    section,
    academicYear,
  ...
  const [classObj, sectionObj] = await Promise.all([
    SchoolClass.findOne({ _id: schoolClass, schoolId }),
    Section.findOne({ _id: section, schoolId }),
  ]);
  ...
  if (existing) {
    throw new ApiError(409, 'A timetable already exists for this class and section. You can edit the existing timetable instead.');
  }
  ...
  if (periods && periods.length > 0) {
    ...
    conflicts = await checkCrossClassConflicts(schoolId, academicYear, null, periods, teachers, subjects);
  ...
  const finalPeriodsPerDay = totalPeriodsPerDay || config.periodsPerDay || 8;
  const finalLunchBreakAfter = lunchBreakAfter || (config.lunchBreaks?.[0]?.afterPeriod) || 4;
  ...
  const timetable = await Timetable.create({
    ...
    academicYear,
    periods: periods || [],
    totalPeriodsPerDay: finalPeriodsPerDay,
    lunchBreakAfter: finalLunchBreakAfter,
    status: status || 'draft',
    isAutoGenerated: false,
    configSnapshot: config.toObject ? config.toObject() : config,
    generationLog: conflicts,
  });
